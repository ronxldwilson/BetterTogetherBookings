...
const TherapistBooking = ({ params }) => {
  ...
  const processPayment = async (e) => {
    e.preventDefault();
    try {
      const orderId = await createOrderId(); // Create the order ID dynamically
      const options = {
        key: process.env.key_id, // Razorpay key
        amount: parseFloat(amount) * 100, // Payment amount in smallest currency unit (e.g., paise for INR)
        currency: currency, // Currency type
        name: 'name', // Name to display on the payment form
        ...
        prefill: {
          name: name, // Prefill customer name
      ...
      paymentObject.open(); // Open the Razorpay payment form
    } catch (error) {
      console.error(error);
    }
  };
